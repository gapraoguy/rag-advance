...
class QASeparateChunkStrategy(FAQChunkStrategy):
    ...
    def create_chunks_for_faqs(self, faqs_by_category: dict) -> list:
        ...
        raise NotImplementedError(
            "This strategy does not support generating a list of chunks from multiple FAQs."
        )
...
FAQ ID: {faq.faq_id}"""
        ...
        return Chunk(text=text, metadata=metadata, chunk_id=f"{faq.faq_id}_question")
...
FAQ ID: {faq.faq_id}"""
        ...
        return Chunk(text=text, metadata=metadata, chunk_id=f"{faq.faq_id}_answer")
